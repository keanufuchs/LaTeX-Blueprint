...
\section{Evaluierung der Alternativen: Nutzwertanalyse}
    ...
    Zur Entscheidungsfindung wurde eine pragmatische Nutzwertbetrachtung durchgeführt. Bewertet wurden fünf Kriterien mit praxisnahen Gewichtungen. Die Skala der Einzelbewertungen reicht von 1 (schwach) bis 5 (sehr gut). Ausschlaggebend waren Typunterstützung, Performance, Validierungsumfang, Fehlertransparenz und Dokumentation (siehe Tabelle~\ref{tabelle:nutzwertanalyse_validierung_konzeption}).
...
Die Struktur von \textbf{XL Fabric} folgt einer klaren Trennung nach Verantwortlichkeiten. Der folgende Verzeichnisbaum illustriert die Organisation der Projektdateien:

\begin{verbatim}
xl-fabric/
|-- config/              # Konfigurationsdateien (Logging, Projekt)
|-- files/
|   |-- input/           # Arbeitsordner fuer Excel-Dateien
|   \-- output/          # Arbeitsordner fuer generierte YAML-Dateien
|-- tests/               # Unit- und Integrationstests
|-- xl_fabric/           # Hauptpaket (Source Code)
|   |-- converter/       # Konvertierungslogik
|   |-- models/          # Pydantic-Modelle (Validierung)
|   |   |-- aciconfig/   # Output-Modelle (Terraform-Struktur)
|   |   \-- xlsx/        # Input-Modelle (Excel-Struktur)
|   |-- utils/           # Hilfsfunktionen
|   |-- cli.py           # Command Line Interface
|   |-- export.py        # YAML-Export-Logik
|   \-- xls.py           # Excel-Parser
...
\label{sec:systemablauf}
    ...
    Ergänzend zur Datenfluss-Sicht visualisiert das Sequenzdiagramm (siehe Anhang, Abbildung~\ref{fig:sequenzdiagramm}) die zeitliche Abfolge und Interaktion der Komponenten während des Validierungsprozesses.
